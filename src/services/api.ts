...
import { Config, isLoliconConfig, isCustomConfig, isNekobotConfig } from '../types/config'
...
export class SetuService {
  ...
  private createProvider(config: Config): ApiProvider {
    if (isLoliconConfig(config)) {
      return new LoliconProvider(config.lolicon.apiUrl, config.lolicon.allowR18)
    } else if (isCustomConfig(config)) {
      return new CustomProvider(config.custom.apiUrl)
    } else if (isNekobotConfig(config)) {
      return new NekobotProvider(config.nekobot.apiUrl, config.nekobot.defaultTag, this.ctx)
    } else {
