...
export function validateParameters(
  ...
  const support =
    APIParameterSupport[provider as keyof typeof APIParameterSupport];
...
export const ConfigSchema = Schema.object({
  ...
  behavior: Schema.object({
    useFigure: Schema.boolean()
      .default(false)
      .description("使用集合回复（解决部分平台发图问题）"),
    replyNumber: Schema.number()
      .default(1)
      .max(10)
      .min(1)
      .step(1)
  ...
  // 幽默
  humor: Schema.object({
    longPicWarning: Schema.boolean()
      .default(false)
      .description("启用错误提示龙图"),
    longPicAddress: Schema.string()
      .default(
        "https://raw.githubusercontent.com/chitsanfei/koishi-plugin-setu/master/assets/long.jpg",
      )
      .description("龙图图片地址"),
  }).description("幽默"),
});
