...
import { Config as ConfigInterface, ConfigSchema, isLoliconConfig, hasValidConfig, validateParameters } from './types/config'
...
export function apply(ctx: Context, config: ConfigInterface) {
  ...
  // 注册指令
  ctx.command('setu', '发一个瑟图')
    .option('size', '-s <string>', { fallback: isLoliconConfig(config) ? config.lolicon.defaultPicSize : 'regular' })
    .option('r18', '-r', { fallback: isLoliconConfig(config) ? config.lolicon.allowR18 : false })
    ...
    .action(async ({ session, options }) => {
      try {
        ...
        // 验证参数
        const size = validatePicSize(options.size) ? options.size : defaultPicSize
        const num = validateReplyNumber(config.behavior.replyNumber) ? config.behavior.replyNumber : 1
        ...
        if (!response.data || response.data.length === 0) {
          ...
          return session.text('.error', [response.error || '没有找到符合条件的图片'])
        }
